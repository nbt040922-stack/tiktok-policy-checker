...
class JudgmentCache {
  ...
  set(key, value) {
    ...
    this.fs.writeFileSync(tempPath, JSON.stringify({ version: 1, entries: Object.fromEntries(this.entries) }), 'utf8');
...
function candidatePayload(policy) {
  return {
    id: policy.id,
    officialTitle: policy.source.document,
    category: policy.category,
    summary: policy.summary,
    ruleText: policy.ruleText,
    outcome: policy.outcome,
    contextualAllowances: policy.contextualAllowances,
    exceptions: policy.exceptions,
    platformTreatment: policy.platformTreatment || null,
    sourceSection: policy.source.section
  };
}

function buildSegmentInput(segments, index, repository, config) {
  ...
  const candidates = repository.getCandidatePolicies({ text: current.text, maxResults: config.candidatePolicies });
  return {
    candidates,
    input: {
      ...
      knownCategories: repository.taxonomy.categories.map(category => category.id)
    }
  };
}
...
function cacheKey(input, modelInfo, config) {
  return stableHash({
    policyVersion: input.policySet,
    promptVersion: POLICY_JUDGE_PROMPT_VERSION,
    model: modelInfo.modelId || modelInfo.model,
...
function applyDecisionSemantics(judgment, config) {
  if (judgment.requiresVisualReview || judgment.decision === 'REVIEW') return 'REVIEW';
  if (judgment.postability === 'PROHIBIT') return judgment.confidence >= config.removeConfidence ? 'REMOVE' : 'REVIEW';
  if (judgment.ageRestricted === true || judgment.fypEligibility === 'PROHIBIT') return 'REVIEW';
  if (judgment.decision === 'REMOVE') return 'REVIEW';
  if (judgment.decision === 'KEEP' && judgment.postability === 'ALLOW' && judgment.confidence >= config.keepConfidence) return 'KEEP';
  return 'REVIEW';
}
...
class PolicyJudgeService {
  constructor({ repository = loadPolicySet(), config = loadPolicyJudgeConfig(), provider, cache } = {}) {
    ...
    this.cache = cache || new JudgmentCache({ maxEntries: config.cacheMaxEntries });
  }
  ...
  async analyzeIngestion(ingestion, { onStage = () => {}, signal, skipHealthCheck = false } = {}) {
    ...
    const latencies = [];
    const metrics = { segmentsTotal: ingestion.transcriptSegments.length, segmentsPrefiltered: 0, segmentsSentToQwen: 0, cacheHits: 0, promptTokens: 0, generatedTokens: 0 };
    onStage('policy');
    ...
    const judgments = await mapLimit(ingestion.transcriptSegments, this.config.concurrency, async (segment, index) => {
      ...
      const { candidates, input } = buildSegmentInput(ingestion.transcriptSegments, index, this.repository, this.config);
      const base = {
      ...
      if (!candidates.length || candidates[0].matchScore < this.config.preFilterMinScore) {
        metrics.segmentsPrefiltered++;
        return { ...base, decision: 'KEEP', decisionSource: 'KEEP_PRECHECK', confidence: 1, postability: 'UNKNOWN', fypEligibility: 'UNKNOWN', monetization: 'UNKNOWN', ageRestricted: null, categories: [], policyIds: [], reason: 'No matching policy signal in transcript.', contextType: 'neutral', requiresVisualReview: false, promptVersion: POLICY_JUDGE_PROMPT_VERSION, policyVersion: this.repository.version, judge: { provider: 'deterministic-prefilter', model: null, quantization: null, promptVersion: POLICY_JUDGE_PROMPT_VERSION }, judgedAt: new Date().toISOString(), error: null, cacheHit: false };
      }
      ...
      if (cached) {
        metrics.cacheHits++;
        return { ...base, ...cached, cacheHit: true };
      }
      metrics.segmentsSentToQwen++;
      const requestStarted = Date.now();
      let judgment;
      try {
        judgment = await this.provider.judgeSegment(input, { signal });
      } catch (error) {
        if (error.code === 'ANALYSIS_CANCELLED') throw error;
        judgment = { decision: 'REVIEW', confidence: 0, postability: 'UNKNOWN', fypEligibility: 'UNKNOWN', monetization: 'UNKNOWN', ageRestricted: null, categories: candidates.slice(0, 1).map(item => item.category), policyIds: candidates.slice(0, 1).map(item => item.id), reason: error.code === 'MODEL_TIMEOUT' ? 'Local model timed out.' : 'Local model output could not be validated.', contextType: 'unclear', requiresVisualReview: false, error: error.code || 'MODEL_ERROR', usage: {} };
      }
      const latency = Date.now() - requestStarted;
      latencies.push(latency);
      metrics.promptTokens += judgment.usage?.promptTokens || 0;
      metrics.generatedTokens += judgment.usage?.generatedTokens || 0;
      const result = {
        decision: applyDecisionSemantics(judgment, this.config),
        decisionSource: 'local-qwen', confidence: judgment.confidence,
        postability: judgment.postability, fypEligibility: judgment.fypEligibility, monetization: judgment.monetization,
        ageRestricted: judgment.ageRestricted, categories: judgment.categories, policyIds: judgment.policyIds,
        reason: judgment.reason.slice(0, 280), contextType: judgment.contextType,
        requiresVisualReview: judgment.requiresVisualReview, error: judgment.error || null,
        promptVersion: POLICY_JUDGE_PROMPT_VERSION, policyVersion: this.repository.version,
        judge: { ...modelInfo, promptVersion: POLICY_JUDGE_PROMPT_VERSION },
        judgedAt: new Date().toISOString(), latencyMs: latency, cacheHit: false
      };
    ...
    onStage('complete');
    return {
      analysisVersion: 'local-qwen-v1', videoId: ingestion.metadata.videoId, url: ingestion.metadata.url,
