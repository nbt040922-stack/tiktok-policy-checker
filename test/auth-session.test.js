...
test('metadata, ingestion, visual proxy, playlist, and download paths use the same auth helper', () => {
  const source = fs.readFileSync(path.join(__dirname, '..', 'main.js'), 'utf8');
  assert.equal((source.match(/youtubeAuth\.withTemporaryCookies/g) || []).length, 5);
